...
export default function PenilaianSASPage() {
  ...
  const handleExport = () => {
    ...
    const fileName = `SAS_${cleanKelas}_${cleanMapel}_${selectedSemester}_${cleanTahunAjaran}.xlsx`;
        ...
        data.forEach((row: any) => {
          ...
          if (studentIndex !== -1) {
            updatedStudents[studentIndex] = {
              ...updatedStudents[studentIndex],
              nilaiAkhir: clampScore(row["SAS"]?.toString() || ""),
            };
          }
        });
    ...
    try {
      for (const student of students) {
        ...
        if (!response.ok) {
          ...
          throw new Error(result?.error || `Gagal mengirim nilai SAS untuk ${student.name}.`);
        }
      }

      setMessage({ text: "Data penilaian SAS berhasil dikirim ke admin!", type: "success" });
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-gray-800">Sumatif Akhir Semester (SAS)</h3>
          <p className="text-sm text-gray-500">Input format penilaian SAS siswa</p>
        </div>
      </div>
            ...
            <table className="border-collapse text-xs w-full">
              <thead className="text-gray-700">
                <tr>
                  ...
                  <th className="border border-gray-400 bg-gray-100 px-2 py-2 w-32 text-center">NILAI SAS</th>
                </tr>
              </thead>
              <tbody>
                {students.map((s, idx) => (

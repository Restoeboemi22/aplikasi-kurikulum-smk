...
export default function PenilaianSikapPage() {
  ...
  const handleExport = () => {
    ...
    const fileName = `Sikap_${cleanKelas}_${cleanMapel}_${selectedSemester}_${cleanTahunAjaran}.xlsx`;
        ...
        data.forEach((row: any) => {
          ...
          if (studentIndex !== -1) {
            updatedStudents[studentIndex] = {
              ...updatedStudents[studentIndex],
              nilaiAkhir: clampScore(row["NILAI SIKAP"]?.toString() || ""),
            };
          }
        });
    ...
    try {
      for (const student of students) {
        ...
        if (!response.ok) {
          ...
          throw new Error(result?.error || `Gagal mengirim nilai Sikap untuk ${student.name}.`);
        }
      }

      setMessage({ text: "Data penilaian Sikap berhasil dikirim ke admin!", type: "success" });
    } catch (error) {
      console.error(error);
    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Input format penilaian Sikap siswa</p>
        </div>
      </div>
            ...
            <table className="border-collapse text-xs w-full">
              <thead className="text-gray-700">
                <tr>
                  ...
                  <th className="border border-gray-400 bg-gray-100 px-2 py-2 w-32 text-center">NILAI SIKAP</th>
                </tr>
              </thead>
              <tbody>
                {students.map((s, idx) => (

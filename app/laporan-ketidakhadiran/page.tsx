...
import { Loader2, AlertCircle, Printer, Filter, Download } from "lucide-react";
...
export default function LaporanKetidakhadiranPage() {
  ...
  const absentList = useMemo(() => {
    ...
    journals.forEach((j) => {
      ...
      const studentParts = studentsStr
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean);
    ...
    return absents.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [journals]);
  ...
  const exportToExcelHandler = async () => {
    if (filteredAbsents.length === 0) return;

    const headers = [
      "Tanggal", "Kelas", "Nama Siswa", "Keterangan", "Guru", "Mata Pelajaran", "Jam ke-"
    ];

    const rows = filteredAbsents.map((item) => {
    ...
    await exportToExcel(
      rows,
      headers,
      `laporan-ketidakhadiran-${new Date().toISOString().split("T")[0]}`,
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            disabled={filteredAbsents.length === 0}
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          {(filterMonth ||
            ...
            filterClass ||
            filterStatus ||
            filterSemester !== defaultPeriod.term ||
            filterAcademicYear !== defaultPeriod.academicYear) && (
            <button
              onClick={resetFilter}
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 w-[200px]">Nama Siswa</th>
          ...
          <tbody className="divide-y">
            ...
            {!loading && filteredAbsents.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-gray-400 text-sm">
                      {journals.length === 0
                        ? "Belum ada data jurnal tidak hadir pada periode yang dipilih."
                        : "Tidak ada data ketidakhadiran siswa untuk filter yang dipilih."}
                </td>
              </tr>
            )}
            {!loading && filteredAbsents.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-4 text-gray-800 font-medium whitespace-nowrap">{item.className}</td>
                ...
                <td className="px-4 py-4 text-gray-600">{item.teacher}</td>

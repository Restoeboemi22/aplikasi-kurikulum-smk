...
import { Eye, CheckCircle2, Clock, Filter, Upload, Check, X, Activity } from "lucide-react";
...
const PAGE_ID = "kurikulum-perangkat-penilaian";
...
const DEFAULT_SUBMISSIONS = [
  { id: 1, teacherName: "Pak Budi Santoso", subject: "Informatika", grade: "X", major: "Teknik Komputer dan Jaringan", majorCode: "TKJ", className: "X TKJ 1", tool: "Bank Soal", status: "Sudah", submissionDate: "10 Juni 2024", verifiedBy: "Wakasek Kurikulum" },
  { id: 2, teacherName: "Bu Siti Aminah", subject: "Matematika", grade: "X", major: "Teknik Komputer dan Jaringan", majorCode: "TKJ", className: "X TKJ 1", tool: "Kisi-kisi", status: "Sudah", submissionDate: "11 Juni 2024", verifiedBy: "Wakasek Kurikulum" },
  { id: 3, teacherName: "Pak Anton Wijaya", subject: "Dasar-Dasar Program Keahlian", grade: "X", major: "Teknik Kendaraan Ringan", majorCode: "TKR", className: "X TKR 1", tool: "Analisis Butir Soal", status: "Belum", submissionDate: "-", verifiedBy: "-" },
  { id: 4, teacherName: "Bu Rina Putri", subject: "Bahasa Inggris", grade: "X", major: "Teknik Komputer dan Jaringan", majorCode: "TKJ", className: "X TKJ 1", tool: "Pedoman Penskoran", status: "Sudah", submissionDate: "12 Juni 2024", verifiedBy: "Wakasek Kurikulum" },
];

const DEFAULT_ACTIVITIES = [
  { id: 1, teacherName: "Pak Budi Santoso", action: "Mengirim Bank Soal", subject: "Informatika", timestamp: "10 Juni 2024, 08:30", type: "submit" },
  { id: 2, teacherName: "Bu Siti Aminah", action: "Mengirim Kisi-kisi", subject: "Matematika", timestamp: "11 Juni 2024, 09:15", type: "submit" },
  { id: 3, teacherName: "Wakasek Kurikulum", action: "Memverifikasi Bank Soal Pak Budi", subject: "Informatika", timestamp: "11 Juni 2024, 10:00", type: "verify" },
];
...
export default function AssessmentToolsPage() {
  ...
  const [submissions, setSubmissions] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('kurikulum-smk-assessment-submissions');
  ...
  const [activities, setActivities] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('kurikulum-smk-assessment-activities');
  ...
  useEffect(() => {
    ...
    const syncData = () => {
      ...
      const savedSubmissions = localStorage.getItem('kurikulum-smk-assessment-submissions');
      if (savedSubmissions) setSubmissions(JSON.parse(savedSubmissions));

      const savedActivities = localStorage.getItem('kurikulum-smk-assessment-activities');
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem('kurikulum-smk-assessment-submissions', JSON.stringify(submissions));
    }
  }, [submissions, isClient]);

  useEffect(() => {
    if (isClient) {
      localStorage.setItem('kurikulum-smk-assessment-activities', JSON.stringify(activities));
    }
  }, [activities, isClient]);
  ...
  const filteredSubmissions = submissions.filter((item: any) => {
    ...
    return teacherMatch && gradeMatch && majorMatch && subjectMatch && toolMatch;
  });
    ...
    <div className="space-y-6">
      ...
      {activeTab === "submit" && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800">Submit Perangkat Penilaian</h3>
              <p className="text-sm text-gray-500">Kirim perangkat penilaian Anda</p>
            </div>
          </div>

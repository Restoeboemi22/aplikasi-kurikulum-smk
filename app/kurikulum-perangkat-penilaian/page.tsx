...
import { Eye, CheckCircle2, Clock, Filter, Upload, Check, X, Activity, Download } from "lucide-react";
...
export default function AssessmentToolsPage() {
  ...
  const loadSubmissionData = async () => {
    const response = await fetch("/api/curriculum-submissions?category=assessment", { cache: "no-store" });
    const result = await response.json().catch(() => null);
    if (!response.ok) {
      throw new Error(result?.error || "Gagal memuat data perangkat penilaian.");
    }
  ...
  const handleSubmitPerangkat = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/curriculum-submissions?category=assessment", {
        method: "POST",
        body: payload,
      });
      const result = await response.json().catch(() => null);
      if (!response.ok) {
        throw new Error(result?.error || "Gagal menyimpan perangkat penilaian.");
      }
      ...
      setFormError(error?.message || "Gagal menyimpan perangkat penilaian.");
    }
  };
  ...
  const handleVerify = async (id: string) => {
    try {
      ...
      const result = await response.json().catch(() => null);
      if (!response.ok) {
        throw new Error(result?.error || "Gagal memverifikasi perangkat penilaian.");
      }
      await loadSubmissionData();
    } catch (error: any) {
      setFormError(error?.message || "Gagal memverifikasi perangkat penilaian.");
    }
  };
